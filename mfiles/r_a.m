...
% psim_m_ay= -[0.3:0.02:3,3:5:100,101:2000:50000];
...
% relative_wetted_surface_ay     =  (r_m_ay./(r_0_ay+radius_particle_m)).^2;
% relative_wetted_surface_ay     = saturation_effective_TSL_ay .* ((r_m_ay)./(r_0_ay+radius_particle_m)).^2;
% relative_wetted_surface_ay     = saturation_effective_TSL_ay .* (r_m_ay./(r_0_ay+radius_particle_m+r_m_ay)).^2;
...
r_s_Van=10*exp(35.63*(0.15-water_content_NSL_ay));

evapo_rate_relative_Sch=1./(1+2/pi*2.3e-4./(rr_a*diffusivity_m2Ps).*(pi/4./water_content_NSL_ay).^0.5.*((pi/4./water_content_NSL_ay).^0.5-1));


r_3_ay                         = r_0_ay./relative_wetted_surface_ay.^0.5;

evapo_rate_relative_capillary = 1./(1  +  r_3_ay.^2 .* thickness_funnel_edl_ay./r_2_ay.^2/(rr_a*diffusivity_m2Ps) + ...
          r_m_ay./2./(rr_a*diffusivity_m2Ps)./relative_wetted_surface_ay .* ...  
...
evapo_rate_relative_vapor = ((rr_a*diffusivity_m2Ps)+thickness_funnel_edl_ay)*tortuosity_0*porosity*log(-psi_0_m) /...
...
surface_resistance_ay_sPm = rr_a*(1./evapo_rate_relative - 1);
% r_s_Sch=rr_a*(1./evapo_rate_relative_Sch - 1);
r_s_Sch=rr_a./evapo_rate_relative_Sch;

%%
%evaporation rate
density_saturated=0.025;
density_ref=0.01;
evaporation_rate_mPs=0.018./(surface_resistance_ay_sPm+rr_a)./1000;
evaporation_rate_mmPday=evaporation_rate_mPs*8.64e7;
evaporation_rate_Sch=0.018./(r_s_Sch+rr_a)./1000.*8.64e7;
evaporation_rate_Van=0.018./(r_s_Van+rr_a)./1000.*8.64e7;


% evaporation_cumulative= sum(evaporation_rate_mPs(1,:))
...
% plot(saturation_NSL_ay,evaporation_rate_mmPday);
% hold on
% plot(saturation_NSL_ay,evaporation_rate_Sch);
% plot(saturation_NSL_ay,evaporation_rate_Van);
% 
% 
semilogy(saturation_NSL_ay,surface_resistance_ay_sPm);
hold on
semilogy(saturation_NSL_ay,r_s_Van);
semilogy(saturation_NSL_ay,r_s_Sch);
